...
class NeuralNetwork:
    ...
    def predict(self):
        ...
        self.layer1 = self.sigmoid(np.dot(self.Data, self.weights1))
...
class GameCore:
    ...
    def crossover(self):
        ...
        while True:
            if len(self.Next_Generation) < self.Population_Number:
                ...
                for e,f in zip(member_1_weights_2, member_2_weights_2):
                    for g,h in zip(e,f):
                        prob = random.random()
                        if prob < 0.47:
    ...
    def GameLoop(self):
        ...
        print(f"Score = {self.Population[-1].Score} Generation Number = {self.Generation_Timer} Number of Birds Living = {len(self.Population)}")
